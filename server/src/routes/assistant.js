...
router.post("/", async (req, res, next) => {
  try {
    ...
    const throttle = await consumeRateLimit(`assistant:${clientIp}`, {
      limit: MAX_REQUESTS,
      windowMs: WINDOW_MS,
    });
    if (throttle.limited) {
    ...
    const digest = createHash("sha256")
      .update(
        `${projectSlug ?? ""}:${question.toLowerCase().replace(/\s+/g, " ")}`,
      )
      .digest("hex");
    const duplicate = await consumeRateLimit(
      `assistant-duplicate:${clientIp}:${digest}`,
      { limit: 1, windowMs: 5_000 },
    );
    if (duplicate.limited) {
      console.warn("[security] assistant_duplicate", { ip: clientIp });
      return res
        .status(429)
        .json({
          message: "This question was just submitted. Please wait a moment.",
        });
    }

    let context;
    try {
      context = await getPortfolioContextWithRetry(projectSlug);
    } catch (error) {
      ...
      return res
        ...
        .json({
          message:
            "Portfolio information is temporarily unavailable. Please try again.",
        });
    }
    if (!context)
      return res
    ...
    if (isStreaming) {
      ...
      try {
        ...
        if (!res.headersSent) {
          ...
          res.write(
            `data: ${JSON.stringify({ error: "Generation interrupted" })}\n\n`,
          );
          return res.end();
        }
        throw error;
